  ...
  module Infra
    class Engine < ::Rails::Engine
      ...
      initializer 'foreman_providers_infra.load_app_instance_data' do |app|
        ForemanProviders::Infra::Engine.paths['db/migrate'].existent.each do |path|
      ...
      initializer 'foreman_providers_infra.register_plugin', :before => :finisher_hook do |_app|
        Foreman::Plugin.register :foreman_providers_infra do
          ...
          role 'ForemanProviders::Infra', [:view_foreman_providers_infra]

          # add menu entry
          menu :top_menu, :template,
               url_hash: { controller: :'foreman_providers_infra/hosts', action: :new_action },
               caption: 'ForemanProviders::Infra',
               parent: :hosts_menu,
               after: :hosts
      ...
      config.to_prepare do
        begin
          Host::Managed.send(:include, ForemanProviders::Infra::HostExtensions)
          HostsHelper.send(:include, ForemanProviders::Infra::HostsHelperExtensions)
        rescue => e
          Rails.logger.warn "ForemanProviders::Infra: skipping engine hook (#{e})"
        end
      end

      rake_tasks do
        Rake::Task['db:seed'].enhance do
          ForemanProviders::Infra::Engine.load_seed
        end
      end
